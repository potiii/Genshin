...
class Calc:
    def data_input(self):
        ...
        print('accessory1_percent?')
        self.accessory1_percent = int(sys.stdin.readline())

    def basic_atk(self):  # 基礎攻撃力+聖遺物+武器%上昇
        base = self.chara_atk + self.weapon_atk  # キャラ攻撃力+武器攻撃力
        accessory = base * (self.accessory1_percent / 100) + self.accessory_atk  # ベース*アクセ倍率+アクセサブステ攻撃力
        weapon = (base * self.weapon_percent / 100)  # ベース*武器倍率
        tmp = base + accessory + weapon
        return tmp

    def data_get(self):
        print(self.basic_atk())
